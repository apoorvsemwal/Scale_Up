...
def show_matched_map(matched_coord_list, fin_data, mode, trip):
    ...
    if len(matched_coord_list) > 0:
        avg_lat = sum(p[0] for p in matched_coord_list) / len(matched_coord_list)
        avg_lon = sum(p[1] for p in matched_coord_list) / len(matched_coord_list)
