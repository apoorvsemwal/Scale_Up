...
/**
 * This is a utility code to test various operations 
 * on Binary Search Trees in Java. 
 * 
...
public class DriverBinarySearchTree {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < seq.length; i++) {
			/** 
			 * Adding new value to BST every time a new Number comes from sequence.
			 * Worst case Comparisons in each loop pass 
			 * 1 + 2 + 3 + 4 + ----- i = O(n^2) - Sum of 1st n numbers - Skewed Tree.
			 * 
			 * Best case Comparisons in each loop pass 
			 * 1 + 2 + 4 + 8 + ----- 2^i = O(log n) - Sum of GP - Balanced tree/Full/Complete Tree.
			 */
	...
	private static void prepareBst(BinarySearchTree bst_tree, int val) {
		...
		if (bst_tree.getRoot() == null) {
			...
			/**
			 * Logic is find the parent of the new value to be inserted
			 * and insert it below that parent.
			 */
	...
	private static void deleteBst(BinarySearchTree bst_tree, int val) {
		...
		/**
		 *  O(h) operation - that is the height of the tree.
		 *  Worst case - O(n) - Skewed Tree
		 *  Best case  - O(log n) - Balanced tree/Full/Complete Tree.
		 */
		...
		if (parent != null) {
			...
			if (del_node != null) {
				...
				/**
				 * Sequence of steps:
				 * 1. Link Parent of Node to be deleted to the Successor
				 * 2. Remove Linkage between successor and its old parent
				 * 3. Set Left  Child of Successor as Left  Child of the node to be deleted
				 * 4. Set Right Child of Successor as Right Child of the node to be deleted
				 * 5. Set Left-Right Children of the node to be deleted as Null.
				 */
				if (!flag_del) {
					if (del_node.getLchild() != null && del_node.getRchild() != null) {
						...
						if (succ != null) {
							...
							if (parent.getLchild().getVal() == val) {
								...
								if (succ_parent.getRchild()!=null && succ_parent.getRchild().getVal()==succ.getVal()){
									succ_parent.setRchild(null);
								}
								if (succ_parent.getLchild()!=null && succ_parent.getLchild().getVal()==succ.getVal()){
	 ...
	 * Successor - Rightmost value on the left subtree of the node.
	...
	/**
	 * Finds the Height of the tree - Length of the longest path from Root to any leaf. 
	...
	private static int findBstHeight(NodeBst root) {
		if (root == null) {
			 ...
			 * Compare both of them and use the greater one for next level.
			 * When a leaf node comes both the below written lines will return 0.
			 * From there on value of height starts building up to root level.
			 */
	 ...
	 * Recursive Solution - O(n^2) - if the tree is skewed - worst case.
	 * Requires a extra variable level - to stop the recursion 
	 * @param root Root node of the BST
	 ...
	 * Requires a Queue - Add element to queue. Pop it and put its children in the queue. 
	 * @param root Root node of the BST
	 */	
	...
	/**
	 * Prints the leaf Nodes - Traverse the tree and print only when both the children are null.
	 * @param root Root node of the BST
	 */	
